...
public class Screen10_Start_A implements Initializable {
    @FXML
    ...
    private void initializeCheckingID() throws SQLException {
        ...
        ResultSet set = Connect.getConnection().createStatement().executeQuery("select bankID, accountID from checking where protectionBank is NULL and protectionAccount is NULL;");
        while (set.next()) {
    ...
    private void initializeSavingsID() throws SQLException {
        ObservableList<String> list = FXCollections.observableArrayList();
        ResultSet set = Connect.getConnection().createStatement().executeQuery("select bankID, accountID from savings where (bankID, accountID) not in (select protectionBank, protectionAccount from checking where protectionBank is not NULL and protectionAccount is not NULL);");
        while (set.next()) {
            list.add(set.getString(1) + ", " + set.getString(2));
        }
