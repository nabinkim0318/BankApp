...
public class Screen10_Stop_C implements Initializable {
    @FXML
    ...
    private void initializeCheckingID() throws SQLException {
        ...
        ResultSet set = Connect.getConnection().createStatement().executeQuery("select bankID, accountID, perID from checking natural join access where protectionBank is not NULL and protectionAccount is not NULL;");
        while (set.next()) {
    ...
    private void onConfirm() throws IOException {
        try {
            CallableStatement statement = Connect.getConnection().prepareCall("{call stop_overdraft(?, ?, ?)}");
