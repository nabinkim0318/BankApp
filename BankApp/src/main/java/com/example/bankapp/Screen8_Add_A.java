...
public class Screen8_Add_A implements Initializable {
    @FXML
    ...
    private void initializeBankAccountID() throws SQLException {
        ...
        ResultSet set = Connect.getConnection().createStatement().executeQuery("select bankID, accountID from bank_account;");
        while (set.next()) {
    ...
    private void onConfirm() throws IOException {
        try {
            ...
            statement.setString(4, BankAccountID.getValue().split(", ")[0]);
            statement.setString(5, BankAccountID.getValue().split(", ")[1]);
            ...
            statement.setString(12, java.time.LocalDate.now().toString());
            statement.execute();
        } catch (SQLException e) {
